<?php
    ...
    $mainContent = '<h2>'.$title.'</h2><hr><center><h2>Overview</h2></center><p>Many packs have Advanced Rocketry in them, but few even use all of Advanced Rocketry\'s default options, let alone all of the other things that can be done with other mods. This guide will cover using some of the basic and advanced options within Advanced Rocketry only to enhance planets in a pack\'s setting and to make them worthwhile to explore for the average player, both in a relatively kitchen-sink pack and in a more advanced pack.</p>
    <br><br>
    <a name="biomes"></a>
    <hr>
    ...
    <p>At a more advanced level, biomes also determine what resources might generate on this planet. They can control resources that only generate in specific biomes normally, or they can become havens for specific resources added to make them more interesting. Consider making barren, lifeless planets not have coal or dirt, as they would never have formed, and adding instead an excess of metallic ores to make up the difference, or any other specific resource. It\'s generally advised to pick biomes based on the temperature of the planet, which can either be calculated, or found in the planetDefs.xml file in a generated world. This avoids weird incongruities such as having a desert world far enough out from a star it should be snowing with its atmosphere condensed into liquid seas, and vice versa with iceworlds that should be Venusian. Temperature varies based on star temperature, size, planetary distance, and planetary atmosphere, so desert worlds could populate the inner orbits of a large star while there are jungle worlds moderately farther out and finally some ice worlds in the distance past the 12AU mark.</p>
    <a name="temperatureatmosphere"></a>
    <hr>
    <center><h2>Temperature and Atmosphere</h2></center>
    <p>Atmosphere and temperature are linked, with too high of a temperature (>450K) causing the atmosphere become the VeryHot atmosphere type and light the player on fire as well as cause slowness unless the player is wearing a space suit. Thus, atmosphere and temperature provide hazards for the player to overcome to access that planet\'s resources. Advanced Rocketry currently has five types of non-air atmospheres, Vacuum, which is any atmosphere <25, Low-Oxygen, which is 25-75, HighPressure, which is 125-800, Super High Pressure, which is 800-1600, and the aforementioned Very Hot atmosphere type that comes with high temperatures. Each atmosphere has its place, and different hazards, as well as different suit requirements. Vacuum, High Pressure, Super High Pressure, and Very Hot all require a full space suit, while LowOxygen only requires a mask and some O2 storage, so normally a chestplate as well. If the atmosphere is >125, geodes will spawn if they are enabled, and if the atmosphere is <25 craters will spawn if they are enabled. Controlling the atmosphere controls both these features and the (though small) challenge players have to face to go around atmospheric effects.</p>
    <a name="uniqueresources"></a>
    <hr>
    <center><h2>Unique Resources</h2></center>
    <img align="left" src="/AdvancedRocketry/img/ferric_sand_processing.jpg" />
