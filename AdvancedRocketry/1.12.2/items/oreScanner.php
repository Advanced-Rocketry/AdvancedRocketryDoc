<?php
    ...
    */    
    ...
    <p>When first crafted the Ore Scanner will not do anything.  To make it work it must be connected to a satellite.  This can be done in the <a href=/AdvancedRocketry/1.12.2/blocks/SatelliteAssemblingMachine.php>Satellite Assembling Machine</a> by using an unprogrammed Ore Scanner in the place of a satellite ID chip.</p>
    <p>If at least 3,000 data storage is available on the satellite, then the scanner can filter by ore.</p>
    <p>If a connected Ore Scanner is placed in the Main Component slot in the <a href=/AdvancedRocketry/1.12.2/blocks/SatelliteAssemblingMachine.php>Satellite Assembling Machine</a> and another Ore Scanner is placed in the copy slot, the connection can be copied to the second ore scanner</p>';
    ...
    <p>Clicking on a square on the display will update the coordinates on the left to indicate the in-world position selected and the value of the ore at that location (higher is better).</p>'; 
