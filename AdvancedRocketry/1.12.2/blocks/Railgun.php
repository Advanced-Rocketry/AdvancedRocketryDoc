<?php
    ...
    */    
    ...
    <p>The railgun requires a tremendous amount of power to accelerate items up to speed, this amount of power is proportional to the distance being fired with a maximum power consumption of 100,000RF.  When firing between planet the energy cost is 500,000 + 10*surface distance.  The railgun can be used to transfer items from one part of the planet to another part of the planet as well at a lower energy cost of 1RF per block distance.</p>
    <p>The railgun itself is comprised of a base which contains components necessary to function such as a power supply, input/output buffers, and a control terminal.  To function, a railgun must have a power supply, control terminal, an input hatch and/or an output hatch.  Naturally a railgun with only an input hatch can only send items, whereas a railgun with an output hatch can only receive items and a railgun with both type can both send and receive items.</p>
    ...
    <p>The slot on the far left must be occupied by a <a href="/AdvancedRocketry/1.12.2/items/linker.php">Linker</a> programmed with the coordinates of the destination railgun.  If being used as receiver only, this slot may be empty.  The linker can be programmed with the coordinates of another railgun simply by shift-right clicking the terminal of another railgun.</p>
    ...
    <p>The power switch is to the left of the redstone control button, if the power switch is in the off position the machine will not fire, however if switched on, then the machine will fire assuming other conditions are met.</p>'; 
